...
* `.map` is a built-in method which returns a new array containing the values returned in the block. `.map` iterates over an existing array and populates the new array using the element that is returned in each iteration.
