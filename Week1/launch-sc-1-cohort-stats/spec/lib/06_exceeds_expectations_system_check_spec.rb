...
describe SystemCheck do
  ...
  describe "#did_student_complete_system_check?" do
    it "returns 'true' if a submission exists for that student" do
      system_check = SystemCheck.new("Grocery List using PostgreSQL", Date.parse("2020/08/28"))
      ...
      expect(system_check.did_student_complete_system_check?(charlie)).to eq(true)
    end

    it "returns 'false' if submission does not exist for that student" do
      system_check = SystemCheck.new("Grocery List using PostgreSQL", Date.parse("2020/08/28"))

      expect(system_check.did_student_complete_system_check?(zoey)).to eq(false)
    end
  end
end
