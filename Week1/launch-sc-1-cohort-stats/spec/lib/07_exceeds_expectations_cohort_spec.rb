...
describe Cohort do
  ...
  describe "#system_check_completed?" do
    ...
    it "returns 'true' if everyone has submitted" do
      system_check = SystemCheck.new("Grocery List using PostgreSQL", Date.parse("2015/08/28"))
      ...
      expect(cohort.system_check_completed?(system_check)).to eq(true)
    end
    ...
    it "returns 'false' if we are missing submissions" do
      ...
      expect(cohort.system_check_completed?(system_check)).to eq(false)
    end
  end
end
